...
use crate::constant;
use crate::host::rmd160::{RMD160Atomic, O, PO, PR, PROUNDS_OFFSET, R, ROUNDS_OFFSET};
use crate::{
...
use crate::utils::{field_to_u32, field_to_u64, Limb};
...
fn get_witnesses<F: FieldExt>(
    ...
    let wb =
        F::from(r as u64) + F::from(rol[0] as u64) + F::from(x as u64) + F::from(offset as u64);
...
impl<F: FieldExt> RMD160Chip<F> {
    ...
    pub fn configure(cs: &mut ConstraintSystem<F>) -> RMD160Config {
        ...
        cs.create_gate("sum with bound", |meta| {
            ...
            let sum_r = r0
                + (r1 + (r2 + r3 * F::from(1u64 << 8)) * F::from(1u64 << 8)) * F::from(1u64 << 8);
            ...
            let offset = config
                .round_config
                .get_expr(meta, RoundGateConfig::offset());
        ...
        cs.create_gate("sum with w1 rol4", |meta| {
            let a_next = config
                .round_config
                .get_expr(meta, RoundGateConfig::a_next());
            let hsel = config.round_config.get_expr(meta, RoundGateConfig::h_sel());
        ...
        cs.create_gate("limbs sum", |meta| {
            ...
            let sum_b = b0
                + (b1 + (b2 + b3 * F::from(1u64 << 8)) * F::from(1u64 << 8)) * F::from(1u64 << 8);
            ...
            let sum_c = c0
                + (c1 + (c2 + c3 * F::from(1u64 << 8)) * F::from(1u64 << 8)) * F::from(1u64 << 8);
            ...
            let sum_d = d0
                + (d1 + (d2 + d3 * F::from(1u64 << 8)) * F::from(1u64 << 8)) * F::from(1u64 << 8);
        ...
        cs.create_gate("c rotate", |meta| {
            ...
            let c_next = config
                .round_config
                .get_expr(meta, RoundGateConfig::c_next());
            ...
            vec![
                (w4h.clone() * constant!(F::from(1u64 << 22)) + w4l.clone() - c.clone())
                    * hsel.clone(),
    ...
    fn assign_next(
        ...
        self.config.round_config.bind_cell(
            region,
            start_offset,
            &RoundGateConfig::a(),
            &previous[0],
        )?;
        let b = self.config.round_config.bind_cell(
            region,
            start_offset,
            &RoundGateConfig::b(),
            &previous[1],
        ...
        let d = self.config.round_config.bind_cell(
            region,
            start_offset,
            &RoundGateConfig::d(),
            &previous[3],
        )?;
        let e = self.config.round_config.bind_cell(
            region,
            start_offset,
            &RoundGateConfig::e(),
            &previous[4],
        ...
        self.config
            .round_config
            .bind_cell(region, start_offset, &RoundGateConfig::x(), input)?;

        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::w1_r(),
            F::from(1u64 << shift[round][index]),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::w1_rr(),
            F::from(1u64 << (32 - shift[round][index])),
        )?;
        ...
        let rol = previous
            .into_iter()
            .map(|c| field_to_u32(&c.value))
            .collect::<Vec<_>>()
            .try_into()
            .unwrap();

        let witness = get_witnesses(
            round,
            &rol,
            field_to_u32(&input.value),
            shift[round][index],
            offset[round],
            pround,
        );
        ...
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::offset(),
            F::from(offset[round] as u64),
        )?;
        ...
        assert!(
            witness.w2b
                == F::from(witness.w1 as u64) + F::from(field_to_u32(&previous[4].value) as u64)
        );
        assert!(
            witness.wb
                == F::from(witness.r as u64)
                    + F::from(field_to_u32(&previous[0].value) as u64)
                    + F::from(field_to_u32(&input.value) as u64)
                    + F::from(offset[round] as u64)
        );
        ...
        self.config.round_config.assign_cell(
            ...
            start_offset,
            &RoundGateConfig::wc(),
            F::from(witness.wc as u64),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::w1(),
            F::from(witness.w1 as u64),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::w1_h(),
            F::from(witness.w1_h as u64),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::w1_l(),
            F::from(witness.w1_l as u64),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::w4_h(),
            F::from(witness.w4_h as u64),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::w4_l(),
            F::from(witness.w4_l as u64),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
        ...
        self.config.round_config.assign_cell(
            ...
            start_offset,
            &RoundGateConfig::w2c(),
            F::from(witness.w2c as u64),
        )?;
        self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::h_sel(),
            F::one(),
        )?;
        let a = self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::a_next(),
            F::from(witness.a_next as u64),
        )?;
        let c = self.config.round_config.assign_cell(
            region,
            start_offset,
            &RoundGateConfig::c_next(),
            F::from(witness.c_next as u64),
        )?;
        Ok([e, a, b, c, d])
    }

    fn rotate_inputs(&self, inputs: &[Limb<F>; 16], round_shift: [usize; 16]) -> [Limb<F>; 16] {
    ...
    pub fn assign_compress(
        ...
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::b(),
            &r0[1],
        ...
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::a1(),
            &r1[0],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::b1(),
            &r1[1],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::c1(),
            &r1[2],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::d1(),
            &r1[3],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::e1(),
            &r1[4],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::a2(),
            &r2[0],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::b2(),
            &r2[1],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::c2(),
            &r2[2],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::d2(),
            &r2[3],
        )?;
        self.config.compress_sum_config.bind_cell(
            region,
            start_offset,
            &CompressSumConfig::e2(),
            &r2[4],
        )?;
        ...
        let anew = {
            ...
            let sum0 = r0[0].value + r1[1].value + r2[2].value;
            let ca0 = (field_to_u64(&sum0) - anew as u64) >> 32;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::sum0(),
                sum0,
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::ca0(),
                F::from(ca0),
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::anew(),
                F::from(anew as u64),
            )?
        };
        ...
        let bnew = {
            ...
            let sum1 = r0[1].value + r1[2].value + r2[3].value;
            let ca1 = (field_to_u64(&sum1) - bnew as u64) >> 32;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::sum1(),
                sum1,
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::ca1(),
                F::from(ca1),
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::bnew(),
                F::from(bnew as u64),
            )?
        };
        ...
        let cnew = {
            ...
            let sum2 = r0[2].value + r1[3].value + r2[4].value;
            let ca2 = (field_to_u64(&sum2) - cnew as u64) >> 32;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::sum2(),
                sum2,
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
        ...
        let dnew = {
            ...
            let sum3 = r0[3].value + r1[4].value + r2[0].value;
            let ca3 = (field_to_u64(&sum3) - dnew as u64) >> 32;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::sum3(),
                sum3,
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::ca3(),
                F::from(ca3),
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::dnew(),
                F::from(dnew as u64),
            )?
        };
        ...
        let enew = {
            ...
            let sum4 = r0[4].value + r1[0].value + r2[1].value;
            let ca4 = (field_to_u64(&sum4) - enew as u64) >> 32;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::sum4(),
                sum4,
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::ca4(),
                F::from(ca4),
            )?;
            self.config.compress_sum_config.assign_cell(
                region,
                start_offset,
                &CompressSumConfig::enew(),
                F::from(enew as u64),
            )?
        };
...
mod tests {
    ...
    use crate::utils::{field_to_u32, Limb};
    ...
    fn test_rmd160_circuit() {
        let test_circuit = RMD160Circuit {
